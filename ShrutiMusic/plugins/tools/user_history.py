...
from pyrogram import filters from pyrogram.types import Message from pyrogram.raw.functions.messages import DeleteHistory

from ShrutiMusic import app, userbot from ShrutiMusic.core.userbot import assistants

@app.on_message(filters.command("sg") & filters.private) async def stalk_user_history(client, message: Message): if len(message.text.split()) < 2 and not message.reply_to_message: return await message.reply("❗ Please provide a username, ID, or reply to a user.")
